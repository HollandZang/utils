...
function jsonFormat(jsonObj, str, retract) {
    str = str === undefined
        ? ""
        : str instanceof Object
            ? ""
            : str
    retract = retract === undefined ? "  " : retract
    const retract1 = retract.substring(0, retract.length - 2)
    if (jsonObj instanceof Object) {
        if (jsonObj instanceof Array) {
            const arr = jsonObj.map(it => jsonFormat(it, str, retract + "  "))
            //数组
            str += "[\n" + `${arr.join(",\n")}` + `\n${retract1}]`
        } else {
            //对象
            ...
            for (let jsonObjKey in jsonObj) {
                if (jsonObj[jsonObjKey] === "") {
                    temp.push(`${retract}"${jsonObjKey}": ""`)
                } else {
                    let p = jsonFormat(jsonObj[jsonObjKey], jsonObj[jsonObjKey], retract + "  ")
                    if (typeof (p) == 'string') {
                        try {
                            const x = JSON.parse(p)
                            if (x instanceof Object) {
                                temp.push(`${retract}"${jsonObjKey}": ${p}`);
                            } else {
                                temp.push(`${retract}"${jsonObjKey}": \"${p}\"`);
                            }
                        } catch (err) {
